# CAM4 Streamlink Multi Model Recorder v.2.0.0 for Python 3.9.1 modified by @horacio9a - All credits to @beaston02 and @ahsand97
# coding: utf-8

import os, sys, time, datetime, configparser, threading, streamlink, subprocess, queue, requests
...
def readConfig():
    ...
    Config.read(mainDir + '/C4_Config.ini')
    setting = {
...
class Modelo(threading.Thread):
    ...
    def run(self):
        ...
        if isOnline == False:
            ...
            self.online = True
            self.file = os.path.join(setting['save_directory'], self.modelo, f'{self.modelo}_C4_{datetime.datetime.fromtimestamp(time.time()).strftime("%d%m%Y-%H%M%S")}.mp4')
            try:
                session = streamlink.Streamlink()
                streams = session.streams(f'{isOnline}')
                stream = streams['best']
                fd = stream.open()
